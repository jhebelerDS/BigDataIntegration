...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
       	// First retrieve the comment and check for num_likes
    	DBCursor db = postsCollection.find(new BasicDBObject("permalink", permalink));
    	
    	// Handle empty count
    	Integer count = 1;
    	
    	while(db.hasNext()){
    		DBObject dod = db.next();
    		BasicDBList comments = (BasicDBList) dod.get("comments");
    		BasicDBObject comment = (BasicDBObject) comments.get(ordinal);
  //    		System.out.println("COMMENT: " + comment);
  		    if( comment.containsField("num_likes")){
    			count = (Integer) comment.get("num_likes");
    			count++;
 //   			System.out.println("COUNT: " + count);
    		}
    		//comments
    		//System.out.println("GET: " + dod.get("comments"))
    		//System.out.println("BOOL: " + dod.containsField("comments.0.author"));
    		//System.out.println("ORD: " + ordinal + "  :"+ dod.toString());
    	}
    	
    	String ordinalStr = Integer.toString(ordinal);
    	WriteResult result = postsCollection.update(new BasicDBObject("permalink", permalink), 
    			new BasicDBObject("$set", new BasicDBObject("comments." + ordinalStr+".num_likes", count)), true, false);
 //   	System.out.println("WR: " + result.getN());
